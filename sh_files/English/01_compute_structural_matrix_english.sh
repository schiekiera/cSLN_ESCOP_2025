#!/bin/bash
#SBATCH --job-name=fasttext_cosine
...
#SBATCH --output=home_directorycsln/data/logs/fasttext_cosine-%j.out
#SBATCH --error=home_directorycsln/data/logs/fasttext_cosine-%j.err
...
# 2. Paths & parameters
# ------------------------------------------------------------------
EMB="home_directorycsln/data/fastText/fastText_english_42k_ukwac.txt"
OUT_BASE="home_directorycsln/data/input/fasttext_english_cosine"
...
# 3. Run the builder
# ------------------------------------------------------------------
python home_directorycsln/py_files/compute_similarity_matrix.py \
  --embeddings      "$EMB" \
  --max-words       "$MAX_WORDS" \
